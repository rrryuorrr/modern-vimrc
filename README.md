...
プラグインを一切使わず、Vim 本体の機能だけで快適さを高めた
Vim 7.4+ / Vim 8.x / Vim 9.x 向け設定です。
端末版 Vim と GVim の両方で同じ `.vimrc` を利用できます。

Vim 7.4ではパッチレベルやビルド機能に応じて、`termguicolors`、
`breakindent`、`signcolumn`、補完候補の非選択表示など、未対応の機能を
自動的に無効化します。公式Vim 7.4.0でも設定の読み込みを確認しています。
